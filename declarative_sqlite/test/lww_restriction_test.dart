...
void main() {
  group('LWW Column Restriction Tests', () {
    ...
    test('allows all columns to be set on new local rows', () async {
      ...
      final results = await database.queryTable('test_items', 
    ...
    test('restricts non-LWW column updates on server-origin rows', () async {
      ...
      final results = await database.queryTable('test_items', 
          where: 'system_id = ?', whereArgs: ['server-row-1']);
      expect(results.length, equals(1));
      ...
      expect(
        ...
        throwsA(isA<StateError>().having(
          (e) => e.message,
          'message',
          contains('is not marked as LWW and cannot be updated on rows that originated from server'),
        )),
      );
    });
    ...
    test('allows all column updates on local-origin rows', () async {
      ...
      expect(() => record.setValue('name', 'Updated Name'), returnsNormally);
      expect(() => record.setValue('description', 'Updated Description'), returnsNormally);
    });

    test('tracks full row for local origin and partial for server origin in dirty rows', () async {
      ...
      await database.update('test_items', 
          {'name': 'Updated Local'}, 
          where: 'system_id = ?', whereArgs: [localSystemId]);

      // Update the server row - should be marked as partial row
      await database.update('test_items', 
          {'name': 'Updated Server'}, 
          where: 'system_id = ?', whereArgs: ['server-row-2']);

      // Check dirty rows
      final dirtyRows = await database.getDirtyRows();
      
      // Find the dirty rows for our test rows
      final localDirtyRow = dirtyRows.firstWhere((r) => r.rowId == localSystemId);
      final serverDirtyRow = dirtyRows.firstWhere((r) => r.rowId == 'server-row-2');
...
class GenericDbRecord extends DbRecord {
  GenericDbRecord(Map<String, Object?> data, String tableName, DeclarativeDatabase database)
