...
class SchemaCodeGenerator {
  ...
  String generateCode(SchemaBuilder schema, {String? libraryName}) {
    final hasFilesetColumns = _hasFilesetColumns(schema);
    ...
    final library = Library((b) => b
      ...
      ..directives.addAll(_generateImports(hasFilesetColumns))
  ...
  /// Checks if the schema contains any fileset columns.
  bool _hasFilesetColumns(SchemaBuilder schema) {
    return schema.tables.any((table) => 
      table.columns.any((column) => column.dataType == SqliteDataType.fileset)
    );
  }

  /// Generates the necessary imports based on what's used in the schema.
  List<Directive> _generateImports(bool hasFilesetColumns) {
    final imports = <Directive>[
      Directive.import('package:declarative_sqlite/declarative_sqlite.dart'),
    ];
    
    if (hasFilesetColumns) {
      imports.add(Directive.import('package:declarative_sqlite_generator/src/fileset_field.dart'));
    }
